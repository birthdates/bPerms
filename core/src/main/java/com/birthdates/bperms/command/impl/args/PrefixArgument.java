...
/**
 * {@link PermissibleArgument} for {@link Permissible#setPrefix(String)}
 *
 * @param <T> Target permissible type
 */
public interface PrefixArgument<T extends Permissible> extends PermissibleArgument<T> {
    ...
    default ArgFunction getFunction(T permissible) {
        return ((player, label, args) -> {
            permissible.setPrefix(String.join(" ", args));
